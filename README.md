...
### TCP Calibration
To calibrate the TCP (i.e. the tip of the extruder) in the URDF (to make sure the model defined in URDF is the same as the actual extruder on the real robot) we can use the [TCP Calibration package](https://github.com/Jmeyer1292/tool_point_calibration). Follow the instructions on the website to setup and use the package, based on the results edit the URDF.
