...
//the gcode file
std::string path_to_file = "../xarm_ws/src/raven/scripts/temp_gcode_file.txt";
...
//defining the string that would be used to control the printing process during the optimization
std::string  optimizer_control = "unknown";
std_msgs::String print_stat_msg;
    

void optimizermessageCallback(const std_msgs::String msg)
{
    //ROS_INFO("optimizermessageCallback received: [%s]", msg.data.c_str());
    optimizer_control = msg.data;
}



// Function to publish the message asynchronously
void publishMessage() {
...
int main(int argc, char** argv) {
    ...
    print_stat_msg.data = "ready";
    ros::Subscriber opt_sub = nh.subscribe("/optimization_node", 1000, optimizermessageCallback);
    
        // Create a separate thread for publishing
    std::thread publishThread(publishMessage);
    ...
    while (optimizer_control != "ready to print"){
        std::cout<< " waiting for the initial segments gcode and trigger " <<std::endl;
        sleep(1);
        }
    ...
    *count = Calculate_file_length(path_to_file);
    ...
    //making the array for trajectory planning and the array with commands for extruder
    ...
    while (optimizer_control != "completed"){
        if (optimizer_control == "ready to print"){
            print_stat_msg.data = "received print trigger";
            // Gcode reading
            *count = Calculate_file_length(path_to_file);
            ...
            //to move the robot to the specifyed extruder orientation
            ...
            for (const auto& SEGMENT : segmentation_array){
                ...
                Gcode_array_of_segment = obj.processSegment("spiral" , "straight" , 0.01 );
                planned_path = obj.path_planner();
